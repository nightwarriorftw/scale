...
def scheduled_interview_email(schedule_id, status):
    ...
    receiver_email = obj.participant
    content_to_template = {'receiver_name': receiver_email,
    ...
    send_email = EmailMessage(
        email_subject, html_content, settings.EMAIL_HOST_USER, [receiver_email])
...
def cancelled_interview_email(obj, status):
    ...
    content_to_template = {'receiver_name': obj.get('receiver_username'),
    ...
    send_email = EmailMessage(
        email_subject, html_content, settings.EMAIL_HOST_USER, [obj.get('receiver_email')])
    send_email.content_subtype = "html"
    send_email.send()
